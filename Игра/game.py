...
def getToken():
    ...
    if os.path.isfile(st.BOT_TOKEN_FILENAME):
        ...
        print("Создать в папке проекта файл 'token.txt' и поместить туда токен для телеграм бота. Перезапустить скрипт)
...
Создать в папке проекта файл 'token.txt' и поместить туда токен для телеграм бота. Перезапустить скрипт
sys.exit()
