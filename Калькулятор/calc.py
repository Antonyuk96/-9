...
def start(update, _):
    update.message.reply_text(f'Привет, {update.effective_user.first_name}, это - калькулятор. Выбор команды.\n' 'Команда /cancel, чтобы отменить действие.\n\n')
...
def choice(update, context):
    ...
    if user_choice in '123':
        if user_choice == '1':
            update.message.reply_text('Ввод числа. \n Первое рациональное число - это: ')
            return RATIONAL_ONE
        if user_choice == '2':
    ...
    if user_choice == '3':
        ...
        def rational_one(update, context):
            ...
            if get_rational.isdigit():
                ...
                update.message.reply_text('Второе число')
                return RATIONAL_TWO
            else:
...
SyntaxError: expected an indented block after function definition on line 37
def operatons_rational(update, context):
    user = update.message.from_user
     logger.info(
